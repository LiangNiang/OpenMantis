            ...
            for i, image_data in enumerate(data_list):
                ...
                if image_url:
                    ...
                    try:
                        ...
                        filename = os.path.join(output_dir, f"image-{timestamp}.jpg")
                    ...
                    if b64:
                        ...
                        try:
                            ...
                            timestamp = int(time.time())
                            filename = os.path.join(output_dir, f"image-{timestamp}.png")
                            with open(filename, "wb") as f:
...
async def image_generate(
    ...
    coroutines = [
        handle_single_task(idx, item, timeout, model, output_dir) for idx, item in enumerate(tasks)
    ]
...
def main():
    ...
    result = asyncio.run(
        image_generate([task], timeout=args.timeout, model_name=args.model, output_dir=output_dir)
    )
