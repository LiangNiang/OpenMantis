...
// Models sometimes return these section fields as a JSON array of bullets
// instead of a single string. Accept both and normalise arrays into a
// markdown-bullet string so downstream rendering stays uniform.
...
规则：
1. 用与对话相同的语言写。
2. 每个 section 字段必须是**字符串**（string），不要用 JSON 数组。多条要点请在同一字符串内用 markdown bullet（每行 \`- \` 开头，行间 \\n）拼接，或用短段落。
