...
Long-term memory is a single flat store (no scoping — collapsed in commit `cc697cf`) with four cognitive types: `semantic`, `procedural`, `episodic`, `prospective`. Each memory is a Markdown file with frontmatter under `$OPENMANTIS_DATA_DIR/memories/<type>/` (path from `MEMORIES_DIR` in `packages/common/src/paths/index.ts`); a single root `MEMORY.md` index is always injected into the system prompt and individual files are read by the agent on demand. Conflict detection (`detectConflictV2`) uses an LLM to flag duplicates/conflicts before `save_memory` writes. Implementation: `packages/core/src/tools/memory/`.
